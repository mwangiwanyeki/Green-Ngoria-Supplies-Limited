...
export function AdminCustomers() {
  ...
  const [editCustomer, setEditCustomer] = React.useState<ErpCustomer | null>(
    null,
  );
  const [viewCustomer, setViewCustomer] = React.useState<ErpCustomer | null>(
    null,
  );
  const [deleteTarget, setDeleteTarget] = React.useState<ErpCustomer | null>(
    null,
  );

  const query = useErpCustomers({
  ...
  const columns: ErpColumn<ErpCustomer>[] = [
    ...
    },
    {
      ...
      cell: (r) => {
        ...
        return (
          <span
            className={`block text-right tabular-nums text-sm ${bal > 0 ? 'font-semibold text-warning-foreground' : 'text-muted-foreground'}`}
          >
            {bal > 0 ? formatKsh(bal) : '—'}
          </span>
        );
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ...
              : 'Add a retail or credit customer to this branch.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3 px-6 pb-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg border border-border p-3 text-center">
              ...
              <p
                className={`text-lg font-bold tabular-nums mt-1 ${balance > 0 ? 'text-warning-foreground' : 'text-success'}`}
              >
